        ...
        if form.is_valid():
            ...
            with transaction.atomic(): # Saving the new data
                ...
                for i in resposta['index']:
                    ...
                    XBM.append(resposta['XBM'][i])
                    XBE.append(resposta['XBE'][i])
...
def e125_pfr_multiplas(request):
    if request.method=="GET":
        form = DadosForm()
        formreacao = ConstForm()
        context = {'form':form, 'formreacao':formreacao},
        return render(request, "e125_pfr_multiplas.html", context=context)
    else:
        form = DadosForm(request.POST)
        formreacao = ConstForm(request.POST)
        if form.is_valid():
            form.save()

            FA0 = request.POST.get("FA")
            FB0 = request.POST.get("FB")
            FC0 = request.POST.get("FC")
            T0 = request.POST.get("T")
        ...
        if form.is_valid():
            ...
            with transaction.atomic(): # Saving the new data
                ...
                for y in resposta['x']:
                    ...
                    dados = E127(pk=i, V=round(V[i],2) , T=round(y[0],2), FA=round(y[1],4), FB=round(y[2],4), FC=round(y[3],4), FD=round(y[4],4), Ta=round(y[5],2))
                    i+=1
                    dados.save()
            qs = E127.objects.all()
...
def e132_batelada(request):
    if request.method=="GET":
        form = E131Form(initial={'Ta':298, 'NA0':9.044,'NB0':33.0,'NM':103.7,'T0':448,'X0':0})
        form.fields = {field: form.fields[field] for field in ['NA0','NB0','NM','T0','Ta','X0']}
        context = {'form':form},
        return render(request, "e132_batelada.html", context=context)
    else:
        form = E131Form(request.POST, initial={'Ta':298, 'NA0':9.044,'NB0':33.0,'NM':103.7,'T0':448,'X0':0})
        form.fields = {field: form.fields[field] for field in ['NA0','NB0','NM','T0','Ta','X0']}
        if form.is_valid():
            form.save()
